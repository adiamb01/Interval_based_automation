...
DMC CHI request-mix capable script:

```bash
Interval_Based_CPU_CMN_DMC_automation_dmc_chi.py
```
...
## 2.1 DMC CHI Request-Mix Only Command

For a focused CPU-pinned `bw_mem` read run:

```bash
python3 Interval_Based_CPU_CMN_DMC_automation_dmc_chi.py \
  --cmd "taskset -c 4 /root/bw_mem_init_once -P 1 -N 1000 -W 2 512m rd" \
  --dmc-phx-chi-reqmix
```

To collect both the original Phoenix FE/BE counters and the CHI request-mix counters:
...
| DMC CHI Request Mix | CHI_REQIF_TRANSFER, read/write retry counters, selected ReadNoSnp and WriteNoSnp opcode counters, read/write mix %, retry % |
...
Optional DMC CHI request-mix collection is enabled with:

```bash
--dmc-phx-chi-reqmix
```

The CHI request-mix pass currently uses `arm_cspmu_mc_*` events across MC ports and reports the following logical metrics:

| Logical metric | Purpose | Current log status |
|---|---|---|
| `CHI_REQIF_TRANSFER` | Total CHI request-interface transfers | Validated non-zero in recent runs |
| `CHI_REQ_XMIT_RD_RETRIES` | CHI read request transmit retries | Collected, currently observed as 0 |
| `CHI_REQ_XMIT_WR_RETRIES` | CHI write request transmit retries | Collected, currently observed as 0 |
| `CHI_REQIF_OP_WRITENOSNPFULL` | WriteNoSnpFull opcode count | Collected, currently observed as 0 |
| `CHI_REQIF_OP_WRITENOSNPFULL_PTL_PCMOSEP` | Partial/PCMO-separate WriteNoSnpFull variant | Collected, currently observed as 0 |
| `CHI_REQIF_OP_WRITENOSNPPTL` | WriteNoSnpPtl opcode count | Collected, currently observed as 0 |
| `CHI_REQIF_OP_WRITEZERO` | WriteZero opcode count | Collected, currently observed as 0 |
| `CHI_REQIF_OP_READNOSNPSEP` | ReadNoSnpSep opcode count | Collected, currently observed as 0 |
| `CHI_REQIF_OP_READNOSNP` | ReadNoSnp opcode count | Collected, currently observed as 0 |

Important implementation note: on the current Phoenix kernel PMU interface, `arm_cspmu_mc_*` accepts `event`, `filter`, `filter2`, and `filter3`; it does **not** accept `impdef`. If perf reports `unknown term 'impdef'`, remove `impdef` from generated DMC event strings.
...
With `--dmc-phx-all`, the summary reports FE/BE pressure counters. With `--dmc-phx-chi-reqmix`, it also reports CHI request-interface transfer and opcode/retry mix counters.
...
DMC CHI request-mix example from recent logs:

```text
DMC CHI request mix
  transfer        : 5310000000 avg / 5350000000 max
  read ops        : 0 avg / 0 max
  write ops       : 0 avg / 0 max
  rd retry        : 0.000% avg / 0.000% max
  wr retry        : 0.000% avg / 0.000% max
  total retry     : 0.000% avg / 0.000% max
  rd/wr mix       : 0.00% read / 0.00% write
```
...
| Metric | Meaning |
|---|---|
| CHI request transfer | Total DMC CHI request-interface activity |
| Read ops | Sum of selected ReadNoSnp opcode counters |
| Write ops | Sum of selected WriteNoSnp/WriteZero opcode counters |
| RD retry % | `CHI_REQ_XMIT_RD_RETRIES / CHI_REQIF_TRANSFER * 100` |
| WR retry % | `CHI_REQ_XMIT_WR_RETRIES / CHI_REQIF_TRANSFER * 100` |
| Read/write mix | Read opcode sum vs write opcode sum |

Current observation: `CHI_REQIF_TRANSFER` is validated and non-zero. The opcode-specific and retry counters are being programmed and collected, but recent logs show them as zero. That likely means the opcode filter encodings still need Phoenix-specific validation, not that perf collection is failing.
...
```bash
grep -E "dmc_phx|dmc_chi" summary.csv
```
